    ...
    if not eval_only:
        ...
        if load_balance > 0:
            ...
            for name, m in model.named_modules():
                if isinstance(m, CustomNaiveGate_Balance_SMoE) or isinstance(m, CustomNaiveGate_Balance_XMoE):
                    balance_loss += m.loss
...
def _train_batch(model, load_balance, optimizer, scheduler, X, Y, h_cache, eval_only, batch_split):
    ...
    if batch_split == 1:
        ...
        h_cache = [
            torch.cat(
                [h_cache_list[i][l] for i in range(batch_split)], dim=0) 
...
def train_iteration(model, load_balance, optimizer, scheduler, data, nb_batches_per_iter, block_size, eval_only, train_pos, h_cache, batch_split, checkpoint_path):
    ...
    for _ in tqdm.tqdm(range(nb_batches_per_iter_max)):
        ...
        if actual_nb_batches_per_iter % 1000 == 1:
            print("Loss: {:.4f} - BPC: {:.4f}".format(loss_all / actual_nb_batches_per_iter, (loss_all / actual_nb_batches_per_iter) / math.log(2)))
            checkpoint_state = {
                'nb_batches_per_iter': actual_nb_batches_per_iter, # last completed iteration
...
def full_eval(model, optimizer, scheduler, data, block_size, hidden_size):
    ...
    h_cache = [
        torch.zeros(data.size(0), model.module.layers[layer_i].attn.attn.get_cache_size(), hidden_size).to(data.device) 
        for layer_i in range(model.module.attn_layer_count)
    ]
